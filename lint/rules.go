...
func Rules() []Rule {
	...
	return []Rule{
		{ID: "ansible-when-parentheses", Summary: "Group nontrivial when conditions", Explanation: "Enclose the complete when condition in parentheses unless it is a single direct variable reference. Check each when list item on structural Ansible owners; preserve nested precedence. Other condition fields and Jinja conditions are outside this convention.", GoodExample: "- name: Example\n  ansible.builtin.debug: {msg: ok}\n  when: (value is defined)\n", BadExample: "- name: Example\n  ansible.builtin.debug: {msg: ok}\n  when: value is defined\n", Kinds: []Kind{Tasks, Handlers, Playbook}, Scope: "structural when fields", Check: checkWhenParentheses},
