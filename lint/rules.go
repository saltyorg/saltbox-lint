...
func Rules() []Rule {
	...
	return []Rule{
		...
		{ID: "ansible-source-header", Summary: "Keep ordered role source headers", Explanation: "Role defaults, tasks, handlers and vars start with the standard ordered header and a document marker within 20 lines. Extra metadata and continuation comments are allowed. Complete unambiguous metadata can be reordered with its comments; incomplete headers remain manual.", GoodExample: "####################\n# Title: Example\n# Author(s): someone\n# URL: https://example.com\n# GNU General Public License v3.0\n---\n[]\n", BadExample: "[]\n", Kinds: []Kind{Defaults, Tasks, Handlers, Vars}, Scope: "file", Fixable: true, Check: checkAnsibleSourceHeader},
		{ID: "docker-healthcheck-shape", Summary: "Use explicit Docker healthcheck block lists", Explanation: "Healthchecks declare exactly one test block list: NONE alone, CMD plus a nonempty executable and optional scalar argv, or CMD-SHELL plus one nonempty scalar command. Numeric and boolean values follow Ansible string normalization; null and collections are invalid. Otherwise valid flow lists with preservable scalar spellings and comments can be converted.", GoodExample: "example_role_docker_healthcheck:\n  test:\n    - CMD\n    - curl\n", BadExample: "example_role_docker_healthcheck:\n  test: [CMD, curl]\n", Kinds: []Kind{Defaults}, Scope: "file", Fixable: true, Check: checkDockerHealthcheckShape},
		...
		{ID: "computed-default-documentation", Summary: "Exclude computed defaults from inventory docs", Explanation: "Owner-local computed _lookup defaults carry one of the two supported documentation-exclusion directives on the immediately preceding physical line. Safe block declarations can receive a Skip docs comment without changing their YAML data.", GoodExample: "# Skip docs\nexample_role_secret_lookup: value\n", BadExample: "example_role_secret_lookup: value\n", Kinds: []Kind{Defaults}, Scope: "file", Fixable: true, Check: checkComputedDefaultDocumentation},
