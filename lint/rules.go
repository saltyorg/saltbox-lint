...
func Rules() []Rule {
	...
	return []Rule{
		{ID: "ansible-when-list", Summary: "Split when conjunctions into block lists", Explanation: "Represent a root logical and in a structural when scalar or list item as separate block-list items. Recursively flatten conjunction groups, preserve operand order, and group nontrivial individual conditions. Keep conjunctions beneath or, negation, comparisons, filters, calls and conditional results intact. Other condition fields and Jinja conditions are outside this convention.", GoodExample: "- name: Example\n  ansible.builtin.debug: {msg: ok}\n  when:\n    - (value is defined)\n    - value\n", BadExample: "- name: Example\n  ansible.builtin.debug: {msg: ok}\n  when: (value is defined) and value\n", Kinds: []Kind{Tasks, Handlers, Playbook}, Scope: "structural when fields", Check: checkWhenList},
		{ID: "ansible-when-parentheses", Summary: "Group nontrivial when conditions", Explanation: "Enclose the complete when condition in parentheses unless it is a single variable read, including direct field/item access or a standalone lookup, query or q call. Operations outside a lookup still need grouping. Root conjunctions belong to ansible-when-list instead. Check each indivisible when list item on structural Ansible owners; preserve nested precedence. Other condition fields and Jinja conditions are outside this convention.", GoodExample: "- name: Example\n  ansible.builtin.debug: {msg: ok}\n  when:\n    - lookup('role_var', '_metrics_enabled', role='traefik')\n    - (value is defined)\n", BadExample: "- name: Example\n  ansible.builtin.debug: {msg: ok}\n  when: value is defined\n", Kinds: []Kind{Tasks, Handlers, Playbook}, Scope: "structural when fields", Check: checkWhenParentheses},
