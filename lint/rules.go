...
func Rules() []Rule {
	...
	return []Rule{
		...
		{ID: "docker-aggregate-contract", Summary: "Compose Docker aggregates in override order", Explanation: "Read explicit role-local default before custom layers; preserve the supported network and host formulas, and keep custom environments solely in their final aggregate combine layer.", GoodExample: "example_role_docker_ports_default: [80]\nexample_role_docker_ports_custom: []\nexample_role_docker_ports: \"{{ lookup('role_var', '_docker_ports_default', role='example') + lookup('role_var', '_docker_ports_custom', role='example') }}\"\n", BadExample: "example_role_docker_networks: []\n", Kinds: []Kind{Defaults}, Scope: "file", Check: checkDockerAggregateContract},
