...
func New(ctx context.Context, opts ...Option) (*Highlighter, error) {
	...
	var lineCache *tokenizer.LineCache
	if cfg.lineCacheBytes > 0 {
		lineCache = tokenizer.NewLineCache(cfg.lineCacheBytes)
	}
	...
	return &Highlighter{
		...
		lineCache:     lineCache,
		documents:     newDocumentCache(cfg.documentCacheBytes),
...
// resultStyleMemo reuses theme selection only while one result is built.
// Themes remain mutable between calls, so this must not live on Highlighter.
type resultStyleMemo map[*theme.Theme]map[string]ast.TokenStyle

func newResultStyleMemo() resultStyleMemo {
	return make(resultStyleMemo)
}

func (m resultStyleMemo) resolve(thm *theme.Theme, scopes []string, key string) ast.TokenStyle {
	styles := m[thm]
	if style, ok := styles[key]; ok {
		return style
	}
	if styles == nil {
		styles = make(map[string]ast.TokenStyle)
		m[thm] = styles
	}
	style := resolveStyle(thm, scopes)
	styles[key] = style
	return style
}

// scopeStackKey preserves every scope boundary and byte in order. A plain
// separator join is ambiguous because scope strings are not restricted here.
func scopeStackKey(scopes []string) string {
	size := len(scopes) * binary.MaxVarintLen64
	for _, scope := range scopes {
		size += len(scope)
	}
	key := make([]byte, 0, size)
	for _, scope := range scopes {
		key = binary.AppendUvarint(key, uint64(len(scope)))
...
func (h *Highlighter) buildResult(
	...
	for i, tokLine := range tokResult.Lines {
		...
		for _, tok := range tokLine {
			...
			scopeKey := scopeStackKey(tok.Scopes)
			ts := memo.resolve(thm, tok.Scopes, scopeKey)
...
func (h *Highlighter) buildResultMulti(
	...
	for i, tokLine := range tokResult.Lines {
		...
		for _, tok := range tokLine {
			...
			scopeKey := scopeStackKey(tok.Scopes)
			def := memo.resolve(defaultThm, tok.Scopes, scopeKey)
			styles := make(map[string]ast.TokenStyle, len(keys)-1)
			for _, k := range keys {
				if k == defaultKey {
					continue
				}
				styles[k] = memo.resolve(thms[k], tok.Scopes, scopeKey)
			}
			themed = append(themed, ast.ThemedToken{
				Content:     content,
				Color:       def.Color,
...
func (h *Highlighter) LoadLanguage(name string, data []byte) error {
	h.documentMu.Lock()
	defer h.documentMu.Unlock()
	if err := h.reg.RegisterGrammar(name, data); err != nil {
		return err
	}
	h.documents.clear()
	h.lineCache.Clear()
