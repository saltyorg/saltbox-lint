...
// WithLineCache enables exact TextMate line-result reuse with a fixed retained
// memory budget in bytes. Cache entries are scoped to this Highlighter and key
// the complete incoming grammar state; 0 disables the cache (the default).
func WithLineCache(maxBytes int) Option {
	return func(o *options) { o.lineCacheBytes = max(0, maxBytes) }
}
...
// WithDocumentCache bounds immutable document checkpoints independently of line reuse.
func WithDocumentCache(maxBytes int) Option {
	return func(o *options) { o.documentCacheBytes = max(0, maxBytes) }
}
