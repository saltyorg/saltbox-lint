...
// taintCache prevents this document's current and subsequent line states from
// entering or consulting the persistent line cache. Tokenization deliberately
// soft-falls back from some scanner errors, so the taint must survive later
// lines whose state was derived from that fallback.
func (m *compileMemo) taintCache() { m.cacheTainted = true }

func (m *compileMemo) cacheSafe() bool { return !m.cacheTainted }
