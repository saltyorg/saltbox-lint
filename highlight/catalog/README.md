...
`make catalog` is the only target that regenerates the catalog. Ordinary
`make check`, `make build`, and `make snapshot` use the checked-in snapshot and
do not require Ansible or its Python environment. To build directly, run:
