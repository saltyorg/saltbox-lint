...
func classify(source string, options Options) ([]Token, error) {
	...
	var err error
	if index == nil || index.Source() != source {
		index, err = yamlindex.Scan(source)
	}
	if err != nil {
...
func (c *classifier) emit(node *yaml.Node, tokenType TokenType, modifiers ...Modifier) {
	...
	start, end, ok := c.index.ScalarRange(node.Line, node.Column, node.Value)
	span := byteRange{start, end}
	if !ok {
		c.err = fmt.Errorf("locate semantic YAML key at %d:%d", node.Line, node.Column)
		return
	}
	if span.start < 0 || span.end <= span.start || span.end > len(c.index.Source()) {
